...
namespace Cvijecara_Sanja_Tica_IT80_2019.Controllers
{
    ...
    { 
        ...
        public ActionResult<StavkaKorpeDto> GetStavkaKorpeById(int id)
        {
            var stavka = stavkaKorpeRepository.GetStavkaKorpeById(id);
            if (stavka == null)
            {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public IActionResult DeleteStavkaKorpe(int id)
        {
            try
            {
                var stavkaKorpeModel = stavkaKorpeRepository.GetStavkaKorpeById(id);
                if (stavkaKorpeModel == null)
                {
                    return NotFound("Stavka korpe sa proslijedjenim id-em nije pronadjena.");
                }
                stavkaKorpeRepository.DeleteStavkaKorpe(id);
                stavkaKorpeRepository.SaveChanges();
                return NoContent();
            }
            catch
        ...
        public ActionResult<StavkaKorpeDto> UpdateStavkaKorpe(StavkaKorpeUpdateDto stavkaKorpe)
        {
            //try
            //{
            var staraStavka = stavkaKorpeRepository.GetStavkaKorpeById(stavkaKorpe.KorpaId);
            if (staraStavka == null)
            {
